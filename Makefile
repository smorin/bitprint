...
setup-py2-part1 := $(shell echo "virtualenv \$${BITPRINT_ENV:-.}/bitprint-virtualenv")
# source the virtual environment so it's active
setup-py2-part2 := $(shell echo ". \$${BITPRINT_ENV:-.}/bitprint-virtualenv/bin/activate")

# command to create the py2 virtualenv
setup-py3-part1 := $(shell echo "python3 -m venv \$$BITPRINT_ENV/bitprint-venv")
# source the virtual environment so it's active
setup-py3-part2 := $(shell echo ". \$${BITPRINT_ENV:-.}/bitprint-venv/bin/activate")
...
setup-%: # On macs will copy text to clipboard so you can just paste it
	...
	@# print the command to create a virtualenv and put it in the $BITPRINT_ENV
	@echo "#Env variable BITPRINT_ENV (should be directory) if not set put it the current directory\n"
	...
	@#  fi
	@echo "export BITPRINT_ENV=~/Desktop/"
...
load-venv-py2-part1 := $(shell echo ". \$${BITPRINT_ENV:-.}/bitprint-virtualenv/bin/activate")
load-venv-py3-part1 := $(shell echo ". \$${BITPRINT_ENV:-.}/bitprint-venv/bin/activate")
...
build: ## builds a setuptool sdist (source distribution and local wheel package) for bitprint
